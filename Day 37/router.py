...
def hot_swap_challenger() -> dict[str, str]:
    ...
    The references are swapped while holding a thread lock,
    preventing race conditions during concurrent predictions.
    """
...
def update_ab_split(
    ...
    if champion_ratio + challenger_ratio != 1.0:
        raise ValueError(
            "Traffic ratios must add up to 1.0."
        )
...
def get_ab_metrics(
    ...
    with model_lock:
        ...
        champion_latencies = list(
            ab_metrics["champion_latencies"]
        )

        challenger_latencies = list(
            ab_metrics["challenger_latencies"]
        )

        champion_avg_latency = (
            float(np.mean(champion_latencies))
