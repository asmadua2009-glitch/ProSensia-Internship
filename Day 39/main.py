...
async def root():
    ...
    return {
        "message": "Day 38 Observable Automated MLOps Pipeline is running.",
        "version": "9.0.0",
    }
...
async def health():
    ...
    return {
        ...
        "challenger_model": lifecycle_status["latest_model"],
...
def run_retraining_pipeline() -> None:
    ...
    try:
        ...
        lifecycle_status["background_retraining"] = True
        ...
        lifecycle_status["retraining_completed"] = True

        print("Background retraining completed.")

        print(f"New model generated: " f"{result['model_path']}")
        ...
        lifecycle_status["model_hot_swapped"] = True

        lifecycle_status["latest_model"] = swap_result["model"]
        ...
        lifecycle_status["ab_split_updated"] = True

        print("A/B traffic split updated.")

        print(f"New split: {split_result}")

    except Exception as exc:

        lifecycle_status["background_retraining"] = False

        print(f"Background retraining failed: {exc}")
...
async def predict(
    ...
    try:
        ...
        model_inference_latency_seconds.observe(inference_latency)
...
async def drift_metrics(
    ...
    try:
        ...
        if drift_result["drift_detected"]:
            ...
            lifecycle_status["drift_alert_triggered"] = True

            if not retraining_in_progress:

                background_tasks.add_task(run_retraining_pipeline)

                drift_result["retraining_triggered"] = True
                ...
                drift_result["retraining_triggered"] = False

                drift_result["message"] = (
                    "Drift detected. " "Retraining already in progress."
                )
            ...
            drift_result["message"] = "No significant data drift detected."

        drift_result["lifecycle"] = lifecycle_status.copy()
        ...
        raise HTTPException(
            status_code=500,
            detail=("Drift detection failed: " f"{exc}"),
        ) from exc
...
async def ab_metrics_endpoint():
    ...
    try:
        ...
        metrics["automated_lifecycle"] = lifecycle_status.copy()

        metrics["retraining_in_progress"] = retraining_in_progress
        ...
        raise HTTPException(
            status_code=500,
            detail=("A/B metrics calculation failed: " f"{exc}"),
        ) from exc
